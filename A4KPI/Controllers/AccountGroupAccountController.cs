...
namespace A4KPI.Controllers
{
    public class AccountGroupAccountController : ApiControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] AccountGroupAccountDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] AccountGroupAccountDto model)
        {
