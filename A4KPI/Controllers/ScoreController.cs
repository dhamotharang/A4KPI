...
namespace A4KPI.Controllers
{
    public class ScoreController : ApiControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult> GetFisrtScoreL1ByAccountId(int accountId, int periodTypeId, int period, string scoreType)
        {
            return Ok(await _service.GetFisrtScoreL1ByAccountId(accountId, periodTypeId, period, scoreType));
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] ScoreDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] ScoreDto model)
        {
