...
namespace A4KPI.Controllers
{
    public class PeriodReportTimeController : ApiControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] PeriodReportTimeDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] PeriodReportTimeDto model)
        {
